...
Pretty much the same as hindsight.py just tailored it to STEVE.
"""
...
The Chrome data folder default locations are:
 ...
 Vista/7/8/10: <userdir>\AppData\Local\Google\Chrome\\User Data\Default\
  ...
  Chromium OS: \home\user\<GUID>\
    ...
    parser.add_argument('-o', '--output', help='Name of the output file (without extension)', required=True)
...
def main():
    def write_excel(analysis_session):
        ...
        output_path = f'{analysis_session.output_name}.{analysis_session.selected_output_format}'
    ...
    try:
        ...
        print(f"Error running analysis session: {e}")
        log.error(f"Error running analysis session: {e}")
        sys.exit(1)
